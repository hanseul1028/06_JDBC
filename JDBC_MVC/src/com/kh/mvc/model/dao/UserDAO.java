 ...
 * 
 * 데이터 베이스 관련된 직업 (CRUD)을 전문적으로 담당하는 객체
...
public class UserDAO {
	 ...
	 * 4) SQL문을 실행 : executeXXX() -> SQL을 인자로 전달하지않음
	 * 									> SELETE(DQL) : executeQuery()
	 * 									> DML : executeUpdate()
	 * 5) 결과 받기 : 
	 * 								  > SELETE : ResultSet타입 객체(조회 데이터 담김)
	 * 									> DML : int(처리된 행의 개수)
	 * 6-1) ResultSet에 담겨있는 데이터들을 하나하나씩 뽑아서 DTO 객체 필드에 
	 * 			옮겨담은 후 조회 결과가 여러 행일 경우 List로 관리
	 * 6-2) 트랜잭션 처리
	 * 7) (생략될 수도 있음) 자원반납(close) => 생성의 역순으로 
	 * 8) 결과 반환 -> Controller
	 * 									> SELECT : 6-1에서 만든거
	 * 									> DML : 처리된 행의 개수
	 * 
